...
RAW_PATH = Path('/home/ubuntu/sushi_raw2.txt')
DATA_PATH = Path('/home/ubuntu/repos/Syrian020/data/vocab.js')
CTX = ['sushi']

SECTION_HEADERS = {
    'سأكمل لاحقًا بـ أسماء أنواع السوشي الكاملة + مكونات كل نوع + عبارات الطلب من الزبائن بنفس الطريقة.',
}

ADJ_HEADS = {
    'Périmé', 'Épicé', 'Doux', 'Doux / Douce', 'Fin', 'Fin / Fine', 'Épais', 'Épais / Épaisse',
    'Cru / Crue', 'Cuite', 'Uniforme', 'Rapide', 'Soigneux', 'Soigneux / Soigneuse', 'Propre', 'Sale',
    'Croustillant', 'Croustillant / Croustillante', 'Croquant', 'Croquant / Croquante', 'Fondant', 'Fondant / Fondante',
    'Frais', 'Frais / Fraîche', 'Congelé', 'Congelé / Congelée', 'Délicieux', 'Délicieux / Délicieuse',
    'Savoureux', 'Savoureux / Savoureuse', 'Salé', 'Salé / Salée', 'Sucré', 'Sucré / Sucrée', 'Acide',
    'Amer', 'Amer / Amère', 'Piquant', 'Piquant / Piquante', 'Parfumé', 'Parfumé / Parfumée',
}
...
PHRASE_HEADS = {
    ...
    'Sur place', 'Une commande à emporter', 'Nettoyer le poste de travail', 'Bienvenue',
    'Couper en morceaux', 'Peser correctement', 'La quantité suffisante', 'La préparation',
    'La précision', 'Le couteau est bien aiguisé', 'Une odeur', 'Une mauvaise odeur',
    'La fraîcheur', 'La qualité', 'Respecter la recette',
}

VERB_HEADS = {
    'Rouler', 'Étaler', 'Répartir', 'Presser', 'Garder',
}
...
EXTRA_RESTAURANT_HEADS = {
    ...
    'Le client', 'La commande', 'Servir', 'Nettoyer', 'Respecter', 'Garder',
    'Rouler', 'Étaler', 'Répartir', 'Presser', 'Couper en morceaux', 'Peser correctement',
    'Respecter la recette', 'Le poulet', 'Le bœuf', 'Le fromage', 'Les légumes', 'La carotte',
    'Le poivron', 'La salade', 'Le maïs', 'Le tofu', 'Le crabe', 'Le poulpe', 'L’anguille',
    'Le thon rouge', 'La sauce piquante', 'La sauce sucrée', 'La sauce épicée', 'Le rouleau de printemps',
    'Les gyozas', 'L’omelette japonaise (tamago)', 'Le fromage de chèvre', 'La mangue',
    'Le fromage à la crème', 'Le surimi', 'Les graines de sésame', 'Le sésame noir', 'Le sésame blanc',
    'La ciboulette', 'La coriandre', 'Le persil', 'La menthe', 'Le citron', 'Le citron vert',
    'Le sel', 'Le sucre', 'L’huile de sésame', 'La farine', 'La chapelure (panko)', 'La tempura',
    'Les œufs de poisson', 'Le caviar', 'La mayonnaise japonaise', 'La sauce teriyaki', 'Le piment',
    'Les oignons frits',
}
...
def infer_level(head, en):
    h = head.lower()
    e = (en or '').lower()
    if any(x in h for x in ['nigiri','sashimi','california','makisu','spatule','charlotte','cuiseur','congélateur','peremption','péremption','fraîcheur','texture','saveur','présentation','temaki','uramaki','futomaki','hosomaki','gyoza','tamago','panko','tempura','caviar','surimi']):
        return 'A2'
    if 'to ' in e and any(x in h for x in ['aiguiser','découper','trancher','mélanger','stocker','congeler','décongeler','peser','mesurer','contrôler','jeter','emballe','emballer','rouler','étaler','répartir','presser','garder']):
        return 'A2'
    return 'A1'
