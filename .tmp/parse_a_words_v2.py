...
# Preprocess: strip known intro/outro lines
all_lines = [l.strip() for l in raw.splitlines()]
lines = []
for l in all_lines:
    if not l:
        continue
    # skip section headers and separators; we'll detect level from headers
    if l == '---' or l.startswith('---'):
        continue
    # keep header for level detection
    lines.append(l)
...
def parse_blocks(lines):
    ...
    while i < n:
        ...
        if not is_french_headword(s):
            i += 1
            continue
        # A French headword must be followed by an Arabic translation line
...
for b in blocks:
    ...
    if key in entries_by_fr:
        ...
        # keep richer example if missing en
        if not old.get("ex"):
            old["ex"] = b["ex"]
        elif b.get("ex", {}).get("en") and not old["ex"].get("en"):
            old["ex"] = b["ex"]
        # update level to the higher CEFR rank between old and new
        if b.get("level") and level_rank(b["level"]) > level_rank(old.get("level", "A2")):
            old["level"] = b["level"]
